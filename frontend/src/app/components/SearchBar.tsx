  ...
  return (
    ...
    >
      <input
        ...
        disabled={isLoading}
        className="flex-grow bg-transparent border-none outline-none text-white placeholder:text-white/50 px-6 py-3 text-base md:text-lg"
      />
      <button
        type="submit"
        disabled={isLoading || !keyword.trim()}
        className="flex items-center justify-center w-[46px] h-[46px] rounded-full bg-white/10 hover:bg-white/20 transition-colors disabled:opacity-50 disabled:cursor-not-allowed shrink-0 mr-1"
        aria-label="Search"
      >
        {isLoading ? (
